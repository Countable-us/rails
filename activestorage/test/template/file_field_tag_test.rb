...
class ActiveStorage::FileFieldTagTest < ActionView::TestCase
  ...
  test "file_field_tag has access to rails_direct_uploads_signed_model_and_attribute" do
    ...
    expected = "<input data-direct-upload-url='http://test.host/rails/active_storage/direct_uploads' data-direct-upload-signed-model-and-attribute='#{rails_direct_uploads_signed_model_and_attribute("User.avatar")}' type='file' name='avatar' id='avatar' />"
    assert_dom_equal expected, file_field_tag("avatar", direct_upload: true, direct_upload_signed_model_and_attribute: "User.avatar")
  end
  ...
  test "file_field has access to rails_direct_uploads_signed_model_and_attribute" do
    ...
    expected = "<input data-direct-upload-url='http://test.host/rails/active_storage/direct_uploads' data-direct-upload-signed-model-and-attribute='#{rails_direct_uploads_signed_model_and_attribute("User.avatar")}' type='file' name='user[avatar]' id='user_avatar' />"
    assert_dom_equal expected, file_field("user", "avatar", direct_upload: true, direct_upload_signed_model_and_attribute: "User.avatar")
  end
